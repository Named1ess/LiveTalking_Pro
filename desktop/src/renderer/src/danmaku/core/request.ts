...
import { makeDanmakuHttpUrl } from './config';
...
export const fetchLiveInfo = async function (id: string) {
  try {
    const html = await fetch(makeDanmakuHttpUrl(`/dylive/${id}`)).then(res => res.text());
    return html;
  } catch (err) {
...
export const fetchUser = async function () {
  try {
    await fetch(makeDanmakuHttpUrl(`/dylive/webcast/user/`), {
      method: 'HEAD',
      headers: {
...
export const fetchImInfo = async function (roomId: string, uniqueId: string) {
  ...
  try {
    ...
    const url = makeDanmakuHttpUrl(`/dylive/webcast/im/fetch/?${makeUrlParams(params)}`);
...
export const fetchMeInfo = async function () {
  try {
    ...
    const url = makeDanmakuHttpUrl(`/dylive/webcast/user/me/?${makeUrlParams(params)}`);
